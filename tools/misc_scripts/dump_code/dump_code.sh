...
GIT_ROOT=$(cd $SCRIPT_PATH && git root)
...
# 5. [hex/sym]:addr for end_addr. End symbol is not yet supported as it is often non-indicative of function end (currently a simple heuristic of next symbol)
end_addr_kind_val="$5"

OBJDUMP=$GIT_ROOT/tools/binutils/bin/arm-none-eabi-objdump

# Define these environmental variables for some customizations

# If defined, do not apply transformations to the objdump output
[ -v $RAW_DUMP ] && [ $RAW_DUMP -ne 0 ] && [ $RAW_DUMP -ne 1 ] && echo "Error: Please input 0 or 1 for RAW_DUMP environmental variable." && exit 1
[ -v $RAW_DUMP ] || RAW_DUMP=0
...
dump0=$($OBJDUMP -D -bbinary -marm7tdmi -Mforce-thumb -z $tmp1 | tail -n +8)
rm $tmp1

if [ $RAW_DUMP -eq 1 ]; then
    dump1=$dump0
else
    dump1=$(
        echo "$dump0" |
...
echo "$symbol_name:"
echo "$dump1"
echo -e "\tthumb_func_end $symbol_name"
